...
public class IzmijeniMaterijalController implements Initializable {
    private int id;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        MaterijalDTO sm = MaterijalController.getSelectedMaterial();
        id = sm.getIdMaterijala();
        naziv = sm.getNazivMaterijala();
        idMaterijalaTextField.setText(sm.getIdMaterijala() == null ? "" : String.valueOf(sm.getIdMaterijala()));
        nazivMaterijalaTextField.setText(sm.getNazivMaterijala() == null ? "" : String.valueOf(sm.getNazivMaterijala()));
        tipMaterijalaTextField.setText(sm.getTipMaterijala() == null ? "" : String.valueOf(sm.getTipMaterijala()));
    }
    ...
    void prihvatiButtonOnAction(ActionEvent event) {
        ...
        if(!naziv.equals(nazivMaterijala) && new MaterijalDAO().dohvatiMaterijalPoNazivuMaterijala(nazivMaterijala) != null) {
        ...
        try {
            ...
            if(idMaterijala != id && new MaterijalDAO().dohvatiMaterijalPoIdentifikatoru(idMaterijala) != null) {
        ...
        if(new MaterijalDAO().azurirajMaterijal(userInput)) {
            success.setHeaderText("Uspješno ste ažurirali materijal");
            success.showAndWait();
            closeStage();
        } else {
            alert.setHeaderText("Greška pri ažuriranju materijala");
            alert.showAndWait();
            clearForm();
            return;
        }
    ...
    private void closeStage(){
        MaterijalController.setSelectedMaterial(null);
